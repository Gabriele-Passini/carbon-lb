...
func NewMockProvider(basePower float64, log *slog.Logger) Provider {
...
func (m *MockProvider) PowerWatts(ctx context.Context) (float64, error) {
	...
	load := 0.3 + 0.4*math.Abs(math.Sin(t/60.0))
...
func NewCAdvisorProvider(cfg config.EnergyConfig, log *slog.Logger) Provider {
	...
	for _, s := range stats {
		if len(s.Stats) >= 2 {
			...
			if dt > 0 {
				...
				estimatedPower := 20.0 + (cpuPct * 0.8)
...
func NewProvider(cfg config.EnergyConfig, basePower float64, log *slog.Logger) Provider {
	switch cfg.Provider {
	case "cadvisor":
